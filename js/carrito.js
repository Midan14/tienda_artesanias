...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document.addEventListener('click', (e) => {
    const cartIcons = document.querySelectorAll('.icon-link .fa-shopping-cart, .cart-icon');
    const cartPreview = document.getElementById('cart-preview');
    
    if (cartPreview) {
      // Verificar si el clic fue en uno de los iconos del carrito o dentro del preview
      const clickedOnCartIcon = Array.from(cartIcons).some(icon => icon.contains(e.target) || icon.parentNode.contains(e.target));
...
function agregarAlCarrito(id, precioOferta) {
  ...
  if (producto) {
    ...
    if (precioOferta) {
      const productoOferta = { ...producto, precio: precioOferta, esOferta: true };
...
function actualizarPreviewCarrito() {
  ...
  if (!previewContainer || !previewTotal) return;
  ...
  if (carrito.length === 0) {
    ...
    if (carrito.length > 3) {
      previewContainer.innerHTML += `<p>Y ${carrito.length - 3} artículos más...</p>`;
    }
